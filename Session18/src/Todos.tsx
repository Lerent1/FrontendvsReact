...
export default function Todos() {
  ...
  const todoReducer=(state:any, action:any)=>{
      switch (action.type) {
        ...
        case "EDIT":
            ...
            let index = state.job.findindex((item: Job) => item.id == action.payload.id);
  ...
  const updateJob({id: number})=> {
    let new_title = prompt("Hay nhap ten cong viec");
    let newJob = ([...item, title: new_title])
  }
  return (
    <div>
      ...
      <h1 > DANH SÁCH CÔNG VIỆC</h1>
       {todos.jobs.map((item:Job,index:number)=>{
        return <li key={index}>{item.title}
         <button onClick={()=>deleteJob(item.id)}>Xóa </button></li>
         <button onClick={()=>updateJob(item.id)}>Sua </button></li>
       })}
    </div>
  )
}