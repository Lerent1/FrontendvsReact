...
abstract class Product {
    ...
    rerestocks(quantity: number): void {
        this.stock += quantity;
...
class Order {
    ...
    totalAmount: number;
    constructor(customer: Customer, products: { product: Product; quantity: number }[], totalAmount: number) {
...
12. Thoát`);
    ...
    for (let choice = 1; choice <= 11; choice++) {
        ...
        switch (choice) {
            ...
            case 4:
                if (store.orders.length > 0) store.cancelOrder(store.orders[0].orderId);
                else console.log("Chưa có đơn hàng nào để hủy!");
                break;
            ...
            case 10:
                ...
                console.log(store.customers.find(c => c.name === "Alice")?.getDetails());
                break;
