...
import type { Task } from "../../utils/listType";
...
export const taskSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        .addCase(updateTask.fulfilled, (state, action)=>{
            return state.map((item: any) => {return item.id==action.payload?.id ? action.payload : item});
        })
        .addCase(toggleTask.fulfilled, (state, action) => {
            return state.map((item: any) => {return item.id==action.payload ? {...item, completed: !item.completed} : item});
        })
    },
});
